...
def train(batchsize, savepath, device, labels=3, epochs=5, mode="rgb"):
    train_dataset = utils.ImageDataset("train", mode)

    val_dataset = utils.ImageDataset("val", mode)
    ...
    I3D.load_state_dict(torch.load('modelWeight/I3D/'+mode+'_imagenet.pt'))
    I3D.replace_logits(labels)

    # optimizer = optim.SGD(I3D_rgb.parameters(), lr=0.01, momentum=0.9,
    #                       weight_decay=0.0001)  # lr=0.0005, weight_decay=0.0000001
    optimizer = optim.Adam(I3D.parameters(), lr=0.0001, weight_decay=0.0001)

    train_loss, train_acc, val_loss, val_acc = utils.trainModel(I3D, train_dataset, val_dataset, batchsize,
                                                                epochs=epochs, optimizer=optimizer, device=device,
                                                                save_path=savepath, save_name='I3D-'+mode)

    utils.saveAndDraw(train_loss, train_acc, val_loss, val_acc, savepath)


def fixed(batchsize, savepath, device, labels=3, epochs=5, mode="rgb"):
    train_dataset = utils.ImageDataset("train", mode)
    ...
    for i in I3D.parameters():
        i.requires_grad = False

    I3D.replace_logits(labels)

    optimizer = optim.Adam(filter(lambda p: p.requires_grad, I3D.parameters()), lr=0.0001, weight_decay=0.0001)
...
def full_ft(batchsize, savepath, device, labels=3, epochs=5, mode="rgb"):
    train_dataset = utils.ImageDataset("train", mode)
...
def test(batchsize, device, labels=3, epochs=5, mode="rgb"):

    test_dataset = utils.ImageDataset("test", mode)
    ...
    utils.testModel(I3D, test_dataset, batchsize, epochs=epochs, device=device)
...
def test_joint(batchsize, device, labels=3, epochs=5):
    ...
    test_dataset = utils.JointDataset(multinum=2)
...
if __name__ == "__main__":
    ...
    batchsize = 15
    mode = "rgb"
    savepath = 'modelWeight/IEMOCAP_I3D_' + mode + '_fixed/'
    device = 'cuda:3'
    epochs = 5

    # train(batchsize, savepath, device, labels, epochs, mode)
    test(batchsize, device, labels, epochs, mode)
    # test_joint(batchsize, device, labels, epochs)
    # fixed(batchsize, savepath, device, labels, epochs, mode)
    # full_ft(batchsize, savepath, device, labels, epochs, mode)
